...
function simpanRiwayatOrder(data_topup, pushname, status, sn, sender) {
    ...
    try {
        ...
        if (!fs.existsSync(riwayatPath + 'buyer/')) {
            fs.mkdirSync(riwayatPath + 'buyer/');
        }

        let riwayat2 = [];

        if (fs.existsSync(riwayatPath +'buyer/' + sender)) {
            const existingData = JSON.parse(fs.readFileSync(riwayatPath +'buyer/' + sender, { encoding: 'utf-8' }));
            riwayat2 = existingData;
        }
        ...
        fs.writeFileSync(riwayatPath + 'buyer/' + sender, JSON.stringify(riwayat2, null, 2));
    } catch (error) {
